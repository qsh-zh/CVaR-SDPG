...
class train_params:
    ...
    ENV = 'InvertedPendulum-v2' #'Walker2d-v2'                     # Environment to use (must have low dimensional state space (i.e. not image) and continuous action space)
    RENDER = False                          # Whether or not to display the environment on the screen during training
    RANDOM_SEED = random.randint(0,100000)                    # Random seed for reproducability
    with io.open(rand_file,"a") as f:
        f.write("train_params use rand: %8i \n"% RANDOM_SEED)
    ...
    REPLAY_MEM_SIZE = 100000       # Soft maximum capacity of replay memory
    ...
    CVaR_alpha = 0.000                    # CVar coefficient
    WANG_SEEKING = 0.0
    WANG_AVERSING = 0.0
    ...
    EXPLORE_TAU = 0.001             # TODO explain this hyper parameter. High EXPLORE_TAU will make it easy to convergent
    ...
    REWARD_LOG_FILE = '/Ant/logs/swg_reward_'+ENV+'_'+str(NUM_ATOMS)+'atom_exp.txt'
...
class test_params:
    ...
    RANDOM_SEED = random.randint(0,100000)                    # Random seed for reproducability
    with io.open(rand_file,"a") as f:
        f.write("test_params use rand: %8i \n"% RANDOM_SEED)
    ...
    # Files/directories
    CKPT_DIR = '/Ant/ckpts/' + ENV                             # Directory for saving/loading checkpoints
    CKPT_FILE = None                                        # Checkpoint file to load and test (if None, load latest ckpt)
    RESULTS_DIR = '/Ant/test_results'                          # Directory for saving txt file of results (if None, do not save results)
    LOG_DIR = '/Ant/test/' + ENV                          # Directory for saving Tensorboard logs (if None, do not save logs)
...
class play_params:
    ...
    RANDOM_SEED = random.randint(0,100000)                    # Random seed for reproducability
    with io.open(rand_file,"a") as f:
        f.write("play_params use rand: %8i \n"% RANDOM_SEED)
