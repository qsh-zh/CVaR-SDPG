'''
## Learner ##
train the D4PG network on experiences sampled (by priority) from the PER buffer
'''
...
class Learner:
    ...
    def run(self):
        ...
        fname_fake = '/Ant/bipedal_fake.txt' 
        fname_real = '/Ant/bipedal_real.txt' 
        ...
        for train_step in range(self.start_step+1, train_params.NUM_STEPS_TRAIN+1):  
            ...
            #samples noise
            noise = np.random.normal(size=[train_params.BATCH_SIZE, train_params.NUM_ATOMS, train_params.NOISE_DIMS])
            ...
            real_samples = self.sess.run(self.critic_target_net.output_samples, {self.state_ph:next_states_batch, self.action_ph:future_action, self.noise_ph: noise})
            ...
            #real_samples = np.repeat(np.expand_dims(real_samples, axis=0), train_params.BATCH_SIZE, axis=0)
            
            # Value of terminal states is 0 by definition
            real_samples[terminals_batch, :] = 0.0
            
            #PRINT fake samples
            
            np.savetxt(fname_fake, np.asarray(100*real_samples), fmt='%4.4f')
            
            # Apply Bellman update to each atom
            real_samples = np.expand_dims(rewards_batch, axis=1) + (real_samples*np.expand_dims(gammas_batch, axis=1))
            
            #PRINT real samples
            
            np.savetxt(fname_real, np.asarray(100*real_samples), fmt='%4.4f')
            
            #real_samples = np.reshape(real_samples, [train_params.BATCH_SIZE, 1])
            
            #samples noise
            noise = np.random.normal(size=[train_params.BATCH_SIZE, train_params.NUM_ATOMS, train_params.NOISE_DIMS])
            
            # TODO, figure this out!!! IMPORTANT!!!
            # Train critic
            TD_error, _ = self.sess.run([self.critic_net.loss, self.critic_train_step], {self.state_ph:states_batch, self.action_ph:actions_batch, self.noise_ph: noise, self.real_samples_ph:real_samples, self.weights_ph:weights_batch})   
            #print(TD_error)
            ...
            action_grads = self.sess.run(self.critic_net.action_grads, {self.state_ph:states_batch, self.action_ph:actor_actions, self.noise_ph: noise})
            ...
            # TODO write corresponding eposide num
            sys.stdout.write('\r{:d}\t'.format(train_step))
            sys.stdout.flush()  
            with io.open(train_params.REWARD_LOG_FILE,'a') as f:
                f.write("Train Step: %10i\n"%(train_step))
