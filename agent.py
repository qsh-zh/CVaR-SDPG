...
from utils.env_wrapper import PendulumWrapper, HumanoidWrapper, LunarLanderContinuousWrapper, BipedalWalkerWrapper, AntWrapper, CheetahWrapper, ReacherWrapper, InvertedPendulumWrapper, HopperWrapper, SwimmerWrapper, Walker2dWrapper
...
class Agent:
    ...
    def __init__(self, sess, env, seed, n_agent=0):
        ...
        if env == 'Pendulum-v0':
            ...
            self.env_wrapper = HumanoidWrapper(env)
        else:
        ...
        while not stop_agent_event.is_set():
            ...
            while not ep_done:
                ...
                if terminal or num_steps == train_params.MAX_EP_LENGTH:
                    ...
                    # Start next episode
                    ep_done = True
                    reward_full.append(episode_reward)
                    reward_full.append(episode_reward)
                    print(episode_reward)
                    # fname1 = '/Ant/swg_reward_walker_1000_exp1.txt' 
                    # np.savetxt(fname1, np.asarray(reward_full), fmt='%4.8f')
                    with io.open(train_params.REWARD_LOG_FILE,'a') as f:
                        f.write("EpisodeReward:%4.8f\n"%(episode_reward))
    ...
    def test(self):   
        ...
        if test_params.RESULTS_DIR is not None:
            ...
            if not os.path.exists(test_params.RESULTS_DIR+'/reward_npy'):
                os.makedirs(test_params.RESULTS_DIR+'/reward_npy')
            np.save(f"{test_params.RESULTS_DIR}/reward_npy/{self.train_ep}",rewards)
    ...
    def noise_test(self,gaussian_noise,noise_level=1.0):
        # Test a saved ckpt of actor network and save results to file (optional)
        def load_ckpt(ckpt_dir, ckpt_file):
            # Load ckpt given by ckpt_file, or else load latest ckpt in ckpt_dir
            loader = self.loader
        ...
        for test_ep in range(1, test_params.NUM_EPS_TEST+1):
            ...
            while not ep_done:
                if test_params.RENDER:
                    self.env_wrapper.render()
                action = self.sess.run(self.actor_net.output, {self.state_ph:np.expand_dims(state, 0)})[0]     # Add batch dimension tosingle state input, and remove batch dimension from single action output
                #print('action')
                #print(action)
                action += gaussian_noise()*noise_level
                #print('action MMM')
                #print(action)
                #print('NOise  ')
        ...
        # Write results to file        
        if test_params.RESULTS_DIR is not None:
            if not os.path.exists(test_params.RESULTS_DIR+ '/' +'noise' +str(noise_level)):
                os.makedirs(test_params.RESULTS_DIR+ '/' +'noise' +str(noise_level))
            if not os.path.exists(test_params.RESULTS_DIR+ '/' +'noise' +str(noise_level)+'/reward_npy'):
                os.makedirs(test_params.RESULTS_DIR+ '/' +'noise' +str(noise_level)+'/reward_npy')
            np.save(f"{test_params.RESULTS_DIR}/noise{str(noise_level)}/reward_npy/{self.train_ep}",rewards)
            output_file = open(test_params.RESULTS_DIR + '/' +'noise' +str(noise_level) + '/' + test_params.ENV + '.txt' , 'a')
            #output_file.write('Training Episode {}: \t Average reward = {:.2f} +/- {:.2f} /ep \n\n'.format(self.train_ep, mean_reward, error_reward))
            output_file.write('{}\t{:.2f}\t{:.2f}\n'.format(self.train_ep, mean_reward, error_reward))
            output_file.flush()
            sys.stdout.write('Results saved to file \n\n')
    ...
    def after_test(self,gaussian_noise,noise_level=1.0):
        # Test a saved ckpt of actor network and save results to file (optional)
        def load_ckpt(ckpt_dir, ckpt_file):
            # Load ckpt given by ckpt_file, or else load latest ckpt in ckpt_dir
            loader = self.loader
        ...
        cnt_loop = 50
        for jj in range(cnt_loop):
            test_params.CKPT_FILE = train_params.ENV+'.ckpt-'+str((jj+1) * int(1000000 / cnt_loop))
            print(f"runing {jj} {test_params.CKPT_FILE}")
            # Load ckpt from ckpt_dir
            load_ckpt(test_params.CKPT_DIR, test_params.CKPT_FILE)
